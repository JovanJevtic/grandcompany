...
initPage(() => {
  ...
  function setSteps(active) {
    $('steps').innerHTML = ['Korpa', 'Dostava i podaci', 'Potvrda'].map((label, i) => {
      ...
      const circle = done ? 'border-sage bg-sage text-canvas' : current ? 'border-ink' : 'border-muted/40';
      return `<li class="flex items-center gap-2 ${done || current ? 'text-ink' : 'text-muted/60'}"${current ? ' aria-current="step"' : ''}>
  ...
  function layoutEmpty() {
    ...
    $('cart-root').innerHTML = `
      <div class="border border-dashed border-ink/30 px-6 py-16 text-center">
        ...
        <p class="mt-3 text-[16px] text-muted">Dodajte artikle iz kataloga ili prenesite gotovu specifikaciju iz kalkulatora.</p>
    ...
    $('cart-root').innerHTML = `
      <div class="grid gap-12 lg:grid-cols-12">
        <div class="lg:col-span-8">
          <section aria-labelledby="h-items">
            <div class="flex items-baseline justify-between gap-4 border-b border-ink pb-3">
              ...
              <button id="btn-clear" class="text-[14px] text-muted underline underline-offset-4 hover:text-steel">Isprazni korpu</button>
            </div>
            <div id="cart-items"></div>
          </section>

          <section aria-labelledby="h-delivery" class="mt-14">
            <div class="border-b border-ink pb-3">${heading('h-delivery', 'Dostava')}</div>
            <div id="cart-delivery" class="mt-6"></div>
          </section>

          <section aria-labelledby="h-details" class="mt-14">
            <div class="border-b border-ink pb-3">${heading('h-details', 'Podaci za narudžbu')}</div>
        ...
        <aside class="lg:col-span-4">
          <div id="cart-summary" class="border border-ink/15 bg-surface p-6 lg:sticky lg:top-40"></div>
        </aside>
      </div>`;
  ...
  function renderItems() {
    ...
    $('cart-items').innerHTML = cartItems().map(({ product: p, qty }) => `
      <div class="grid grid-cols-[88px_1fr] gap-x-5 gap-y-3 border-b border-ink/10 py-5 sm:grid-cols-[124px_1fr_auto]">
        <a href="${productUrl(p)}" class="block aspect-[4/3] self-start bg-well p-1" tabindex="-1" aria-hidden="true">${productArt(p)}</a>
        <div class="min-w-0">
          <a href="${productUrl(p)}" class="text-[16px] font-semibold leading-snug hover:text-steel">${esc(p.name)}</a>
          <p class="mt-0.5 text-[13px] text-muted">${p.sku}, ${esc(p.spec)}</p>
          <p class="mt-1 text-[14px]">${KM(priceOf(p))} po ${p.unit}${b2b ? ` <span class="ml-1 text-muted line-through">${KM(p.price)}</span>` : ''}</p>
          <div class="mt-3 flex flex-wrap items-center gap-4">
            ${cartStepper(p, qty)}
            <button data-remove="${p.sku}" class="text-[14px] text-muted underline underline-offset-4 hover:text-steel">Ukloni</button>
          </div>
        </div>
        <p class="tnum col-start-2 text-[20px] font-semibold leading-none sm:col-start-3 sm:text-right">${KM(priceOf(p) * qty)}</p>
      </div>`).join('');
  }
  ...
  function renderDelivery() {
    ...
    $('cart-delivery').innerHTML = `
      <div class="grid gap-3 md:grid-cols-3">
        ${options.map((o) => `
          <label class="flex cursor-pointer flex-col border p-4 transition-colors ${state.delivery === o.id ? 'border-ink bg-surface' : 'border-ink/20 hover:border-ink/50'}">
            <span class="flex items-center justify-between gap-3">
              <input type="radio" name="delivery" value="${o.id}" ${state.delivery === o.id ? 'checked' : ''} class="border-ink/40 text-steel focus:ring-steel" />
              <span class="text-[14px] font-medium">${o.cost}</span>
            </span>
            <span class="mt-3 text-[16px] font-semibold leading-snug">${o.title}</span>
            <span class="mt-1 text-[14px] leading-snug text-muted">${o.text}</span>
          </label>`).join('')}
      </div>
      ...
      <p class="mt-5 text-[15px] text-muted">Procijenjena težina pošiljke: <strong class="font-medium text-ink">${fmt0.format(t.weightKg)} kg</strong></p>
      ${t.weightKg >= CRANE_RECOMMEND_OVER_KG && state.delivery !== 'kran'
        ? '<p class="mt-2 border-l-2 border-steel pl-3 text-[15px] text-muted">Pošiljka prelazi tonu, pa preporučujemo kamion sa kranom.</p>'
        : ''}`;
  ...
  function renderPayment() {
    ...
    if (p) {
      ...
      if (!fits) {
        warning = `<p class="border-l-2 border-steel pl-3 text-[15px] text-muted">Narudžba od ${KM(t.total)} prelazi slobodan kreditni limit od ${KM(free)}. Izaberite avansno plaćanje ili zatražite povećanje limita od komercijaliste.</p>`;
      }
    } else {
      options = [
  ...
  function renderSummary() {
    ...
    if (p) {
      ...
      credit = `
        <div class="mt-5 border-t border-ink/15 pt-4 text-[14px] text-muted">
          <p>Kreditni limit nakon ove narudžbe</p>
          <div class="mt-2 h-1.5 overflow-hidden bg-ink/15"><div class="h-full ${usedPct < 85 ? 'bg-sage' : 'bg-steel'}" style="width:${usedPct}%"></div></div>
          <p class="mt-2">Slobodno još <strong class="font-medium text-ink">${KM(Math.max(0, p.creditLimit - creditUsed(p) - t.total))}</strong> od ${KM(p.creditLimit)}</p>
        </div>`;
    }
    ...
    $('cart-summary').innerHTML = `
      ...
      <div class="mt-4 border-t border-ink/15 pt-3">
        ${t.rebate > 0 ? row('Vrijednost po cjenovniku', `<span class="line-through">${KM(t.grossB2C)}</span>`, 'text-[15px] text-muted') : ''}
        ${t.rebate > 0 ? row(`Partnerski rabat −${pct(discount())}`, `−${KM(t.rebate)}`, 'text-[15px] text-steel') : ''}
        ...
        ${row('Osnovica bez PDV-a', KM(t.net), 'text-[13px] text-muted')}
        ${row('PDV 17%', KM(t.vat), 'text-[13px] text-muted')}
      </div>
      <div class="mt-3 flex items-baseline justify-between border-t border-ink pt-4">
        <span class="text-[15px]">Ukupno</span>
        <span class="tnum text-[30px] font-semibold leading-none">${KM(t.total)}</span>
      </div>
      ${credit}
      <button type="submit" form="checkout-form" class="${BTN_PRIMARY} mt-6 w-full py-4 text-[16px]">Pošalji narudžbu</button>
      <p class="mt-3 text-[13px] leading-relaxed text-muted">Narudžba stiže komercijalisti, koji je potvrđuje i šalje predračun iz Pantheona. Ništa se ne naplaćuje prije potvrde.</p>`;
  }
  ...
  function submitOrder(e) {
    ...
    const detail = (label, value) => `<div><dt class="text-[13px] text-muted">${label}</dt><dd class="mt-1 text-[16px]">${esc(value)}</dd></div>`;
    ...
    $('cart-root').innerHTML = `
      <div class="grid gap-12 lg:grid-cols-12">
        <div class="lg:col-span-7">
          ...
          <p class="mt-4 max-w-[58ch] text-[17px] leading-relaxed text-muted">Komercijalista provjerava stanje i ${next}, zove vas na ${esc(phone)} i šalje predračun. Atesti i deklaracije stižu uz robu.</p>
          <dl class="mt-8 grid gap-5 border-t border-ink/15 pt-6 sm:grid-cols-2">
        ...
        <aside class="lg:col-span-5">
          <div class="border border-ink/15 bg-surface p-6">
            <h3 class="font-serif text-[22px]">Naručeni artikli</h3>
            <ul class="mt-3 divide-y divide-ink/10 border-t border-ink/15">
              ${t.items.map(({ product: pr, qty }) => `
                <li class="flex items-center gap-3 py-3">
                  <span class="block h-11 w-14 shrink-0 bg-well p-0.5">${productArt(pr)}</span>
                  <span class="flex-1 text-[14px] leading-snug">${esc(pr.name)}<br /><span class="text-muted">${qtyFmt(qty)} ${pr.unit}</span></span>
                  <span class="whitespace-nowrap text-[14px] font-medium">${KM(priceOf(pr) * qty)}</span>
                </li>`).join('')}
            </ul>
            <div class="mt-3 space-y-1 border-t border-ink/15 pt-3 text-[15px]">
              ${t.rebate > 0 ? `<div class="flex justify-between text-steel"><span>Partnerski rabat</span><span>−${KM(t.rebate)}</span></div>` : ''}
              <div class="flex justify-between"><span>Dostava</span><span>${t.deliveryCost ? KM(t.deliveryCost) : 'besplatno'}</span></div>
            </div>
            <div class="mt-3 flex items-baseline justify-between border-t border-ink pt-3">
              <span>Ukupno sa PDV-om</span><span class="tnum text-[24px] font-semibold leading-none">${KM(t.total)}</span>
            </div>
          </div>
        </aside>
      </div>`;
